...
const asRecord = (value: unknown): RawRow => (value && typeof value === 'object' ? (value as RawRow) : {});
...
const normalizeEmployee = (row: RawRow): Employee => {
  ...
  const normalizedOther = Object.fromEntries(
    Object.entries(otherAllowances).map(([key, value]) => [key, toNumber(value)]),
  );
  ...
  return {
    ...
    onboarding_status: row.onboarding_status === 'completed' ? 'completed' : 'pending',
    magic_link_token: row.magic_link_token ? String(row.magic_link_token) : null,
    basic_salary: basic,
    hra,
...
const normalizePayrollRun = (row: RawRow): PayrollRun => {
  ...
  return {
    ...
    draft_data: row.draft_data ? (draftData as Record<string, PayrollInputs>) : null,
...
export async function createBusiness(name: string, userId: string) {
  const { data, error } = await supabase
    .from('businesses')
    .insert({ name, owner_id: userId, state: 'Maharashtra', onboarding_step: 1 })
    .select()
    .single();

  if (error) throw error;
  return normalizeBusiness(asRecord(data));
}
...
export async function updateBusiness(
  businessId: string,
  updates: Partial<Pick<Business, 'name' | 'state' | 'logo_url' | 'onboarding_step'>>,
) {
  const { data, error } = await supabase
    .from('businesses')
    .update(updates)
...
export async function updateOnboardingStep(businessId: string, step: 1 | 2 | 3) {
...
export async function addEmployee(businessId: string, employee: EmployeeInput) {
  const { data, error } = await supabase
    .from('employees')
    .insert({ ...employee, business_id: businessId, onboarding_status: employee.onboarding_status ?? 'completed' })
    .select()
    .single();
...
export async function updateEmployee(employeeId: string, updates: Partial<EmployeeInput>) {
